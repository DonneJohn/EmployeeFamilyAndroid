...
import com.exmobile.employeefamilyandroid.adapter.CultureAdapter;
import com.exmobile.employeefamilyandroid.bean.Culture;
import com.exmobile.employeefamilyandroid.presenter.ListCulturePresenter;
 ...
 */
@RequiresPresenter(ListCulturePresenter.class)
public class CultureListFragment extends BaseListFragment<Culture, ListCulturePresenter> implements BaseListAdapter.OnItemClickListener {
    ...
    @Override
    protected BaseListAdapter<Culture> onSetupAdapter() {
        return new CultureAdapter(getContext(), BaseListAdapter.ONLY_FOOTER);
    }

    @Override
    public void onItemClick(int position, long id, View view) {
        String api = AppManager.LOCAL_LOGINED_USER.getAPI_IP();
        UIManager.gotoDetail(getContext(), api.substring(0, api.length() - 4) + "Appweb/CultureDetail.aspx?RowID=" + mAdapter.getItem(position).getRowID(), "企业文化详情");
    }
...
//        Log.d("thanatos", "change adapter state");
     ...
     * @param result
     */
    public void onLoadResultData(List<Culture> result) {
...
//        Log.d("thanatos", "onLoadResultData - result's size = " + result.size());
        ...
        Iterator<Culture> iterator = result.iterator();
        final List<Culture> data = mAdapter.getDataSet();
        while (iterator.hasNext()) {
            Culture obj = iterator.next();
