 ...
 */
public class ListRulePresenter extends BaseListPresenter<RuleListFragment> {
    public static final String CACHE_KEY_All = "AllRuleListData";

    public String cache_key = CACHE_KEY_All;

    private static final int REQUEST_RULE_DATA = 1;
    ...
    protected void onCreate(Bundle savedState) {
        ...
        this.<Integer, Subscription, Integer, Object>restartable(REQUEST_RULE_DATA, (pageNum, subscriber, mode, o4) -> {
            Observable<RespRule> observable = ServerAPI.getEmployeeAPI(AppManager.LOCAL_LOGINED_USER.getAPI_IP()).getRuleList(pageNum + 1, AppManager.LOCAL_LOGINED_USER.getFK_Company());
            return observable.subscribeOn(Schedulers.io())
                    .compose(this.<RespRule>deliverFirst())
                    ...
                    .subscribe(split(
                            (view, page) -> {
                                ...
                                if (page != null && page.getResult() != null && page.getResult().size() > 0)
                                    this.<Rule>cacheData(view.getContext(), page.getResult(), cache_key);
                                view.onLoadResultData(page.getResult());
                                view.onLoadFinishState(RuleListFragment.LOAD_MODE_DEFAULT);
                            },
                            (view, error) -> {
     ...
     * @param view
     */
    private void init(RuleListFragment view) {
        // 读取缓存
        view.onLoadingState(BaseListFragment.LOAD_MODE_CACHE);
        Subscription mReadCacheSubscriptor = this.<List<Rule>>getCacheFile(view.getContext(), cache_key)
     ...
     * @param subscriptor
     */
    public void requestData(RuleListFragment view, Subscription subscriptor) {
        // 判读网络情况
        ...
        start(REQUEST_RULE_DATA, 0, subscriptor, BaseListFragment.LOAD_MODE_DEFAULT, null);
    }
    ...
    public void requestData(int mode, int pageNum) {
        ...
        start(REQUEST_RULE_DATA, pageNum, null, mode, null);
    }
     ...
     * @param subscriptor
     */
    public void dismissReadCache(RuleListFragment view, Subscription subscriptor) {
