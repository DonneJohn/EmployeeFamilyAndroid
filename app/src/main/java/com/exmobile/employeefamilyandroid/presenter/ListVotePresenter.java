 ...
 */
public class ListVotePresenter extends BaseListPresenter<VoteListFragment> {
    public static final String CACHE_KEY_All = "AllVoteListData";

    public String cache_key = CACHE_KEY_All;

    private static final int REQUEST_VOTE_DATA = 1;
    ...
    protected void onCreate(Bundle savedState) {
        ...
        this.<Integer, Subscription, Integer, Object>restartable(REQUEST_VOTE_DATA, (pageNum, subscriber, mode, o4) -> {
            Observable<RespVote> observable = ServerAPI.getEmployeeAPI(AppManager.LOCAL_LOGINED_USER.getAPI_IP()).getVoteList(pageNum + 1, AppManager.LOCAL_LOGINED_USER.getRowID(),AppManager.LOCAL_LOGINED_USER.getFK_Department());
            return observable.subscribeOn(Schedulers.io())
                    .compose(this.<RespVote>deliverFirst())
                    ...
                    .subscribe(split(
                            (view, page) -> {
                                ...
                                if (page != null && page.getResult() != null && page.getResult().size() > 0)
                                    this.<Vote>cacheData(view.getContext(), page.getResult(), cache_key);
                                view.onLoadResultData(page.getResult());
                                view.onLoadFinishState(VoteListFragment.LOAD_MODE_DEFAULT);
                            },
                            (view, error) -> {
     ...
     * @param view
     */
    private void init(VoteListFragment view) {
        // 读取缓存
        view.onLoadingState(BaseListFragment.LOAD_MODE_CACHE);
        Subscription mReadCacheSubscriptor = this.<List<Vote>>getCacheFile(view.getContext(), cache_key)
     ...
     * @param subscriptor
     */
    public void requestData(VoteListFragment view, Subscription subscriptor) {
        // 判读网络情况
        ...
        start(REQUEST_VOTE_DATA, 0, subscriptor, BaseListFragment.LOAD_MODE_DEFAULT, null);
    }
    ...
    public void requestData(int mode, int pageNum) {
        ...
        start(REQUEST_VOTE_DATA, pageNum, null, mode, null);
    }
     ...
     * @param subscriptor
     */
    public void dismissReadCache(VoteListFragment view, Subscription subscriptor) {
        if (!subscriptor.isUnsubscribed()) {
            view.onLoadFinishState(RuleListFragment.LOAD_MODE_CACHE);
